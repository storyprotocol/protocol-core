...
contract RoyaltyPolicyLAP is IRoyaltyPolicyLAP, Governable, ERC1155Holder, ReentrancyGuard {
    ...
    struct LAPRoyaltyData {
        ...
        uint32 royaltyStack; // royalty stack for a given ipId is the sum of royalties to be paid to all its parents
    ...
    function onLicenseMinting(
        ...
        if (data.splitClone == address(0)) {
            ...
            // the license for this royalty policy is being minted for the first time parentIpIds are zero given
            // that only roots can call _initPolicy() for the first time in the function onLicenseMinting() while
            // derivatives already called _initPolicy() when linking to their parents with onLinkToParents() call.
            ...
            // and that targetAncestors passed in matches the record stored in state on policy initialization
            if (
