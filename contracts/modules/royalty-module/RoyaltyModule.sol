...
contract RoyaltyModule is IRoyaltyModule, Governable, ReentrancyGuard, BaseModule {
    ...
    /// @param _licenseRoyaltyPolicy The royalty policy of the license being minted
    ...
    function payLicenseMintingFee(
        ...
        address _licenseRoyaltyPolicy,
        address _token,
        uint256 _amount
    ) external onlyLicensingModule {
        if (!isWhitelistedRoyaltyToken[_token]) revert Errors.RoyaltyModule__NotWhitelistedRoyaltyToken();

        if (_licenseRoyaltyPolicy == address(0)) revert Errors.RoyaltyModule__NoRoyaltyPolicySet();
        if (!isWhitelistedRoyaltyPolicy[_licenseRoyaltyPolicy])
            revert Errors.RoyaltyModule__NotWhitelistedRoyaltyPolicy();

        IRoyaltyPolicy(_licenseRoyaltyPolicy).onRoyaltyPayment(_payerAddress, _receiverIpId, _token, _amount);
