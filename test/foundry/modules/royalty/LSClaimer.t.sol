...
contract TestLSClaimer is BaseTest {
    ...
    function _setLsClaimer() internal {
        ...
        royaltyPolicyLS.initPolicy(_getIpId(mockNFT, nftIds[0]), parentsIds1, abi.encode(10));

        // set up derivative royalty policy
        for (uint256 i = 0; i < 99; i++) {
            address[] memory parentsIds = new address[](1);
            parentsIds[0] = _getIpId(mockNFT, nftIds[i]);
            royaltyPolicyLS.initPolicy(_getIpId(mockNFT, nftIds[i + 1]), parentsIds, abi.encode(10));
        }
        vm.stopPrank();

        (address split, address claimer, uint32 rStack, uint32 mRoyalty) = royaltyPolicyLS.royaltyData(
            _getIpId(mockNFT, nftIds[99])
        );
        ...
        for (uint256 i = 0; i < 100; i++) {
            LONG_CHAIN[i] = _getIpId(mockNFT, nftIds[i]);
        }
        assertEq(LONG_CHAIN[0], _getIpId(mockNFT, nftIds[0]));
        assertEq(LONG_CHAIN[99], _getIpId(mockNFT, nftIds[99]));
    ...
    function test_LSClaimer_claim_revert_AlreadyClaimed() public {
        address claimerIpId = _getIpId(mockNFT, nftIds[0]);
        tokens[0] = USDC;
    ...
    //     accounts[0] = _getIpId(mockNFT, nftIds[99]);
    ...
    function test_LSClaimer_claim() public {
        ...
        address claimerIpId = _getIpId(mockNFT, nftIds[0]);
        (address claimerSplitClone, , , ) = royaltyPolicyLS.royaltyData(_getIpId(mockNFT, nftIds[0]));
        tokens[0] = USDC;
